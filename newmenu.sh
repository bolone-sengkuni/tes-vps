...
Exp=$(curl -sS https://raw.githubusercontent.com/kulo-sinten/tes-vps/main/REG/ipvps| grep $MYIP | awk '{print $3}')
fi
chck_b(){
...
curl -sS https://raw.githubusercontent.com/scvpn/scriptvps/main/clgshow
echo -e "
"
fi
echo -e "\033[0;34m??????????????????????????????????????????????????\033[0m"
...
if [[ $(cat /opt/.ver) = $serverV ]] > /dev/null 2>&1; then
    ...
    if [[ $x -eq 999 ]]; then
       wget -q -O /usr/bin/update-script "https://raw.githubusercontent.com/kulo-sinten/tes-vps/main/dll/system/update-script.sh" && chmod +x /usr/bin/update-script
