...
cekdata=$(curl -sS https://raw.githubusercontent.com/scvpn/user-backup-db/main/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
...
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/scvpn/user-backup-db/main/$NameUser/$NameUser.zip" &> /dev/null
