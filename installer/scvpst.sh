...
Disable_Order() {
   [[ "${message_from_id[$id]}" == "$Admin_ID" ]] && {
     ...
     [[ "$(grep -wc "off" "/tmp/order")" = '1' ]] && {
         ...
         ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
             --text "?? Successfully Enabled Order ??" \
             --parse_mode html
         sleep 1
         ShellBot.deleteMessage	--chat_id ${message_chat_id[$id]} \
              --message_id $bdx
        ...
        fun_online() {
            ...
            for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
                ...
                [[ $conex -gt '0' ]] && {
                    ...
                    info2+="?????????????????????\n"
                    info2+="<code>?? $user      ? $conex      ? $timerr</code>\n"
                }
            done
            echo -e "$info2"
        }
...
ShellBot.InlineKeyboardButton --button 'sett_menus' --line 2 --text '?? See Log Reseller ??' --callback_data '_seelog'
...
unset menu
menu=''
ShellBot.InlineKeyboardButton --button 'menu' --line 1 --text '?? Open Service ???' --callback_data '_openserv'
ShellBot.InlineKeyboardButton --button 'menu' --line 1 --text '?? Status Service ????' --callback_data '_stsserv'
ShellBot.InlineKeyboardButton --button 'menu' --line 2 --text '?? Current Price ??' --callback_data '_priceinfo'
ShellBot.InlineKeyboardButton --button 'menu' --line 2 --text '?? Settings Menu ??' --callback_data '_menusettss'
ShellBot.InlineKeyboardButton --button 'menu' --line 10 --text '?? Unblock User ??' --callback_data '_unblck'
...
unset menu_re_main
menu_re_main=''
ShellBot.InlineKeyboardButton --button 'menu_re_main' --line 1 --text '?? Open Service ???' --callback_data '_pps_serv'
ShellBot.InlineKeyboardButton --button 'menu_re_main' --line 2 --text '?? Status Service ???' --callback_data '_sts_serv'
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                '?? CREATE USER ??\n\nUsername:')
                    ...
                    [[ "$(awk -F : '$3 >= 1000 { print $1 }' /etc/passwd | grep -w ${message_text[$id]} | wc -l)" != '0' ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "?? User Already Exist..")" \
                            --parse_mode html
                        >$CAD_ARQ
                        break
                        ShellBot.sendMessage --chat_id ${callback_query_message_chat_id[$id]} \
                    ...
                    ShellBot.sendMessage --chat_id ${message_from_id[$id]} \
                        --text 'Password:' \
                        --reply_markup "$(ShellBot.ForceReply)" # For?a a resposta.
                    }
                    ;;
                'Password:')
                    verifica_acesso
                ...
                'Saldo:')
                    ...
                    [[ ${message_text[$id]} != ?(+|-)+([0-9]) ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "?? Use only numbers [EX: 100000]")" \
                            --parse_mode html
                        break
                    }
